...
class UjianSkbController extends Controller
{
	...
	$setting = SettingUjian::where('is_active', '=', 'Y')->first();
        if ($count > 0) {
            return view('skb.hasil', compact('hasil'));
        } else {
            return view('skb.index', compact('peserta', 'setting'));
        }
    }
	...
	if($id < 1 || $id > $setting->jumlah_soal || $count_peserta < 1) {
        ...
        //for ($i = 0, $j = 0; $i < 100; $i++) {
	for ($i = 0, $j = 0; $i < $setting->jumlah_soal; $i++) {
            if ($jawaban_user[$i] == '0') {
                $j++;
            }
        }
        ...
        if($request->ajax()) {
            return response()->json([
                'view' => view('skb.tes',compact('soal', 'peserta', 'setting'))
    ...
    public function store(Request $request)
    {
	    ...
	    $setting = SettingUjian::where('is_active', 'Y')->first();
            $soal = SoalSkb::select('id','jawaban')->where('jabatan_id', Auth::guard('peserta-skb')->user()->jabatan_id)->inRandomOrder()->take($setting->jumlah_soal)->distinct()->get();
            ...
            //for ($i = 0; $i < 100 ; $i++) {
	    for ($i = 0; $i < $setting->jumlah_soal ; $i++) {
                $jawaban_kosong[$i] = '0';
            }
	...
	//    return redirect()->route('login-skb.form');
	//}
