...
<nav id="sidebar">
    <div class="list-group list-group-flush overflow-auto vh-100 mt-1">
        @php
            $nomor_soal = 1;
            $kosong = 0;
	    $row_navigation = $setting->jumlah_soal / 5;
        @endphp
        @for ($i = 1; $i <= $row_navigation; $i++)
                ...
                <ul class="nav navbar-nav ml-auto">
                    <li class="nav-item">
                        <div class="alert alert-success text-center mr-4" style="width: 180px; height: 45px;">
                            Soal Dijawab: {{ $setting->jumlah_soal - $kosong }}
                        </div>
                    </li>
                    <li class="nav-item">
...
@push('ajax')
    <script>
        ...
        $('#btn-save').click(function(event) {
            ...
            let urlMax = "{{ route('ujian-skb.show', $setting->jumlah_soal) }}";
...
@push('time')
	...
	var waktuJS = waktuJS + ({{ $setting->waktu }} * 60 * 1000);
